...
def profile_setup_menu(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("📁 Выбрать из папки", callback_data="profile_select_source_folder")],
        [InlineKeyboardButton("📋 Все аккаунты", callback_data="profile_select_source_all")],
        [InlineKeyboardButton("🔙 Назад", callback_data="menu_accounts")]
    ]
    
    text = "⚙️ Настройка профиля\n\nВыберите источник аккаунтов:"
...
def process_bulk_links_async(accounts, base_link: str, method: str, status_message, chat_id) -> dict:
    ...
    try:
        ...
        system_max_workers = min(limits.max_workers, 4)  # Не более 4 для ссылок
...
def process_bulk_photos(update: Update, context: CallbackContext) -> int:
    ...
    max_workers = min(5, len(account_ids))  # Максимум 5 параллельных потоков
...
def get_profile_handlers():
    ...
    handlers = [
        ...
        profile_selector.get_conversation_handler(),  # Добавляем обработчик селектора
    ]
