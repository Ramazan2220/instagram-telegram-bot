...
def get_publish_handlers():
    ...
    story_conversation = ConversationHandler(
        entry_points=[
            # Убираем MessageHandler из entry_points чтобы не перехватывать все фото
            # MessageHandler будет только в states после выбора аккаунтов
        ],
        states={
            UPLOAD_MEDIA: [
...
def handle_story_media_upload(update, context):
    ...
    # Дополнительная проверка: если пользователь в режиме редактирования профиля, игнорируем
    if 'selected_profile_accounts' in context.user_data or 'bulk_photos' in context.user_data:
        logger.info(f"📱 STORY: Пользователь в режиме редактирования профиля - игнорируем")
        return None
